...
//- (void) isTwitterAvailable:(CDVInvokedUrlCommand*)command{
    ...
    //pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsInt:twitterSDKAvailable ? 1 : 0];
...
- (void) getXReverseAuth:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options{
    NSString *callbackId =[arguments objectAtIndex:0];
    NSString *x_reverse_auth_target = [options objectForKey:@"x_reverse_auth_target"];
    NSString *x_reverse_auth_parameters = [options objectForKey:@"x_reverse_auth_parameters"];
    NSString *username = [options objectForKey:@"username"];
    NSLog(@"Auth start:\n%@",username);
    //NSDictionary *obtainAccessTokenParams = [[NSDictionary alloc] init];
    //[obtainAccessTokenParams setValue:x_reverse_auth_target forKey:@"x_reverse_auth_target"];
    //[obtainAccessTokenParams setValue:x_reverse_auth_parameters forKey:@"x_reverse_auth_parameters"];
    
        
    NSURL *url1 = [NSURL URLWithString:@"https://api.twitter.com/oauth/request_token"];
    NSURL *url2 = [NSURL URLWithString:@"https://api.twitter.com/oauth/access_token"];
    //NSURL *url2 = [NSURL URLWithString:@"https://api.twitter.com/1.1/account/verify_credentials.json"];
   // SLRequest *stepTwoRequest =
    //[SLRequest requestForServiceType:SLServiceTypeTwitter requestMethod:SLRequestMethodPOST URL:url2 parameters:obtainAccessTokenParams];
    ...
    [accountStore requestAccessToAccountsWithType:twitterType
                withCompletionHandler:^(BOOL granted, NSError *error) {
                    if (!granted) {
                        ...
                        for (objectInstance in accounts){
                            ACAccount *twitterAccount = [accounts objectAtIndex:indexKey++];
                            //ACAccountCredential *twCre = [twitterAccount credential];
                            NSString *un = twitterAccount.username;
                            if ([un isEqualToString:username]){
                                NSLog(@"Running time%d",indexKey);
                                account = twitterAccount;
                                break;
                            }
                        }
                        TWAPIManager *m = [[TWAPIManager alloc] init];
                        [m performReverseAuthForAccount:account withHandler:^(NSData *responseData, NSError *error) {
                            NSString *responseStr =
                            [[NSString alloc] initWithData:responseData encoding:NSUTF8StringEncoding];
                            NSLog(@"The user's performr:\n%@", responseStr);
                            NSString *jsResponse =[[CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                                                                     messageAsString:responseStr ] toSuccessCallbackString:callbackId];
...
- (void) verifyAuth:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options{
    NSString *callbackId =[arguments objectAtIndex:0];
    ...
    [accountStore requestAccessToAccountsWithType:twitterType
                            withCompletionHandler:^(BOOL granted, NSError *error) {
                                if (!granted) {
                                    ...
                                    NSLog(@"Auth dfdffdf:\n%@",username);
                                    TWRequest *req =[[TWRequest alloc] initWithURL:url2 parameters:nil requestMethod:TWRequestMethodGET];
                                    req.account = account;
                                    NSLog(@"Authuser:\n%@",url2);
                                    [req performRequestWithHandler:^(NSData *responseData, NSHTTPURLResponse *urlResponse, NSError *error) {
                                        NSLog(@"Res:\n%@",urlResponse);
                                        if (error != nil){
                                            NSString* newStr = @"There is an error!";
                                            
                                            NSLog(@"Auth dfdffdf:\n%@",newStr);
                                            NSString *jsResponse = [[CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
                                                                                      messageAsString:newStr]
...
- (void) getTwitterUsernames:(NSMutableArray*)arguments withDict:(NSMutableDictionary*)options{
    NSString *callbackId = [arguments objectAtIndex:0];
    ACAccountStore *accountStore = [[ACAccountStore alloc] init];
    ACAccountType *accountType = [accountStore accountTypeWithAccountTypeIdentifier:ACAccountTypeIdentifierTwitter];
    ...
    [accountStore requestAccessToAccountsWithType:accountType withCompletionHandler:^(BOOL granted, NSError *error) {
        if(granted) {
            ...
            NSMutableDictionary *allAccountDic = [[NSMutableDictionary alloc] init];
            NSMutableArray *array =[[NSMutableArray alloc] init];
            for (objectInstance in accountsArray){
                ACAccount *twitterAccount = [accountsArray objectAtIndex:indexKey];
                //ACAccountCredential *twCre = [twitterAccount credential];
                //NSString *accessToken =[twCre oauthToken];
                NSString *username = twitterAccount.username;
                NSMutableDictionary *row =[[NSMutableDictionary alloc] init];
                //[row setObject:(accessToken) forKey:(@"accessToken")];
                [row setObject:(username) forKey:(@"username")];
                [allAccountDic setObject:row forKey:[NSString stringWithFormat:@"UserIndex%d", indexKey++]];
                [array addObject:row];
            }
